...
_ALWAYS_INLINE_ size_t fnv1a_hash(const char *p_data, size_t p_len) {
...
_ALWAYS_INLINE_ size_t hash(const std::string_view &p_value) {
	return fnv1a_hash(p_value.data(), p_value.size());
}

_ALWAYS_INLINE_ size_t hash(int p_value) {
	return p_value;
}

template <class T>
_ALWAYS_INLINE_ void hash_combine(std::size_t &p_hash, const T &p_value) {

...
inline void _start_name_value_pair_new(String &p_arr, const char *p_name, const String &p_value) {
	p_arr += "\"";
	p_arr += p_name;
	p_arr += "\":\"";
	p_arr += p_value;
...
inline void _next_name_value_pair_new(String &p_arr, const char *p_name, const String &p_value) {
	p_arr += ",\"";
	p_arr += p_name;
	p_arr += "\":\"";
	p_arr += p_value;
...
String build_view_hierarchy_json_optimized() {
	...
	while (!stack.is_empty()) {
		...
		_start_name_value_pair_new(json, "name", node->get_name());
		_next_name_value_pair_new(json, "class", node->get_class());

		String scene_path = node->get_scene_file_path();
		if (!scene_path.is_empty()) {
			_next_name_value_pair_new(json, "scene", scene_path);
		}

		const Ref<Script> &scr = node->get_script();
		if (scr.is_valid()) {
			_next_name_value_pair_new(json, "script", scr.is_valid() ? scr->get_path() : String());
		}

...
#if defined(SDK_NATIVE)
	{
		...
		if (platform_info.wine_proton.is_wine) {
			internal_sdk->set_tag("compatibility",
					platform_info.wine_proton.is_proton ? "proton" : "wine");
			if (!platform_info.wine_proton.wine_version.is_empty()) {
				internal_sdk->set_tag("wine_version", platform_info.wine_proton.wine_version);
			}
			if (platform_info.wine_proton.is_proton && !platform_info.wine_proton.proton_version.is_empty()) {
				internal_sdk->set_tag("proton_version", platform_info.wine_proton.proton_version);
			}
		}
